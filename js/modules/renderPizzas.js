...
const createCard = (data) => {
  ...
  card.innerHTML = `
    <picture>
      ...
      <img class="card__image" src="${data.images[0]}" alt="Пицца ${data.name.ru}" />
    </picture>
    ...
    <div class="card__content">
      ...
      <button class="card__button" data-id="${data.id}" aria-label="Выбрать пиццу" type="button">Выбрать</button>
    </div>
  `;
...
export const renderPizzas = async (toppings) => {
  const pizzas = await getData(
    `https://boiling-sprout-griffin.glitch.me/api/products${toppings ? `?toppings=${toppings}` : ''}`,
  );
  ...
  if (pizzas.length) {
    ...
    modalController({
      ...
      async cbOpen(btnOpen) {
        const pizza = await getData(`https://boiling-sprout-griffin.glitch.me/api/products/${btnOpen.dataset.id}`);
